...
void Comm5Serial::requestProtocolVersionResponseHandler(const std::string & frame)
{
	uint8_t protocol = frame[5];
	uint8_t version = frame[6];
	uint8_t capabilities = frame[7];
	uint8_t inputCount = frame[8];
	uint8_t outputCount = frame[9];
...
bool Comm5Serial::writeFrame(const std::string & data)
{
	...
	std::string frame({ 0x05, 0x64, length, 0x00 });
	frame.append(data);
	uint16_t crc = 0;
	for (auto c : frame)
...
{
	//                  id    op     mask
	std::string data({ 0x02, 0x01, (char)0xFF });
	writeFrame(data);
}
...
{
	//                  id    mask
	std::string data({ 0x01, (char)0xFF });
	writeFrame(data);
}
...
{
	//                  id    op   on/off  mask
	std::string data({ 0x04, 0x02, 0x01, (char)0xFF });
	writeFrame(data);
}
